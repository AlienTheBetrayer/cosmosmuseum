...
export const ReactedButton = ({
  ...
  if (!reactions?.length) {
    return (
      <span className="flex h-8 items-center justify-center rounded-full border border-white/10 bg-black/30 px-3 text-xs font-medium text-white/60 shadow-sm backdrop-blur-md">
        Реакцій немає
      </span>
    );
  }
  ...
  return (
    <AnimatePresence initial={false}>
      <motion.ul layout className="flex items-center gap-0.5">
          ...
          >
            <Button
              className={cn(
                ...
                reaction.liked &&
                  "border-blue-400/30 bg-blue-500/20 shadow-[0_0_12px_rgba(59,130,246,0.15)]",
              )}
              onClick={() => {
                add.mutate({
                  id: nanoid(),
